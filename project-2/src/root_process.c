...
//User defined macros: to account for storing file path/hash, dup_list/retain_list lengths, storing file descriptor as a string
...
void redirection(char **dup_list, int size, char *root_dir)
{
    ...
    //Close file descriptors & error check
    if (close(fd) == -1)
    {
    ...
    if (close(TEMP_STDOUT_FILENO) == -1)
    {
        perror("Failed to close temporary file descriptor");
        exit(-1);
    }
}
...
int main(int argc, char *argv[])
{
    ...
    if (pid == 0)
    {
        //Set pipe so child can write to it
        if (close(fd[0]) == -1)
        {
            perror("First nonleaf failed to close read end of pipe");
            exit(-1);
        }

        //Execute first nonleaf
        char fd_name[FD_MAX];
        ...
        perror("First nonleaf failed to execute nonleaf process");
        exit(-1);
    }
    else if (pid > 0)
    {
        //Set pipe so parent can read from it
        if (close(fd[1]) == -1)
        {
            perror("Root process failed to close write end");
            exit(-1);
        }
        ...
        if (close(fd[0]) == -1)
        {
            perror("Root process failed to close read end");
            exit(-1);
        }
    }
    else
